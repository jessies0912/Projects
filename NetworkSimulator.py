...
class NetworkSimulator:
    ...
    def initSimulator(self, maxMsgs, loss, corrupt, delay, seed, trace):
        ...
        # initialise sender entity (the respective class)
        self.sender = sender(A, self)

        # initialise receiver entity (the respective class)
    ...
    def runSimulator(self):
        ...
        for i in range(0, 1000):
            # random.random() should be uniform in [0,1]
        ...
        while True:
            # Get the next (with respect to the scheduled time) event in the
            # list
            ...
            # Advance the simulator's time to be the scheduled time of the next
            # event
            ...
            if next_event.event_type == EventType.TIMERINTERRUPT:
                ...
                if next_event.entity == A:
                    ...
                    if self.trace >= 1:
                        print(
                            "B: Receiving the data and sending the acknowledgement " + next_event.packet.payload)
                ...
                # If we've reached the maximum message count, exit the main
                # loop
                if (self.nMsgSim < self.maxMessages):
                    ...
                    # do not schedule more FROMAPP events from the application
                    # layer if we reached the maximum number of messages
            else:
    ...
    def generateNextArrival(self):
        ...
        # arrival time 'x' is uniform on [0, 2 * avgMessageDelay] having mean
        # of avgMessageDelay.
        ...
        if self.trace >= 2:
            ...
            print("generateNextArrival(): future time for " + "event " + str(next_event.event_type) +
                  " at entity " + str(next_event.entity) + " will be " + str(next_event.time))
    ...
    def startTimer(self, entity, increment):
        ...
        if (t != None):
            ...
            timer_event = Event(self.time + increment,
                                EventType.TIMERINTERRUPT, entity)
    ...
    def stopTimer(self, entity):
        ...
        if (timer_event == None):
            if self.trace >= 1:
                print(
                    "stopTimer: Warning: Unable to cancel your timer, which is not set.")
    ...
    def udtSend(self, entity, p):
        ...
        # Set destination to be the 'other side' of the network; B if we are A
        # or vice versa.
        if entity == A:
            destination = B
        elif entity == B:
        ...
        # Since the medium cannot reorder, the packet will arrive 1 to 10 time
        # units after the last packet sent by this sender
        ...
        if self.trace >= 2:
            print(
                "udtSend: Scheduling arrival on other side: at time   " + str(arrivalTime))

        arrival = Event(arrivalTime, EventType.FROMNETWORK,
                        destination, packet)
